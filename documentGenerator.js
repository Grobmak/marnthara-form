...
function calculateGrandTotal(payload) {
    return payload.rooms.reduce((roomSum, room) => {
        ...
        const setsTotal = room.sets.reduce((sum, set) => sum + CALC.calculateSetPrice(set), 0);
        const decosTotal = room.decorations.reduce((sum, deco) => sum + CALC.calculateDecoPrice(deco), 0);
        const wpsTotal = room.wallpapers.reduce((sum, wp) => sum + CALC.calculateWallpaperPrice(wp), 0);
...
export function generateQuotationHtml(payload, options) {
    ...
    payload.rooms.forEach(room => {
        ...
        room.sets.forEach(set => {
            const totalSetPrice = CALC.calculateSetPrice(set);
            if (totalSetPrice > 0) {
        ...
        room.decorations.forEach(deco => {
            const decoPrice = CALC.calculateDecoPrice(deco);
            if (decoPrice > 0) {
        ...
        room.wallpapers.forEach(wp => {
            const wpPrice = CALC.calculateWallpaperPrice(wp);
            if (wpPrice > 0) {
                const totalWidth = wp.widths.reduce((a, b) => a + b, 0);
                const rolls = CALC.wallpaperRolls(totalWidth, wp.height_m);
