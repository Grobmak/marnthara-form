(function() {
    ...
    function recalcAll() {
        ...
        document.querySelectorAll(SELECTORS.room).forEach(room => {
            ...
            room.querySelectorAll(SELECTORS.wallpaperItem).forEach(wallpaper => {
                let totalItemPrice = 0, materialPrice = 0, installPrice = 0;
                let rollsNeeded = 0;
    ...
    function renumber() {
        document.querySelectorAll(SELECTORS.room).forEach((room, rIdx) => {
            ...
            items.forEach((item, iIdx) => {
                ...
                if (titleEl) titleEl.textContent = `${iIdx + 1}/${items.length}`;
            });
        });
    }
    ...
    function updateLockState() {
        ...
        document.querySelectorAll('input, select, textarea, button').forEach(el => {
            const isExempt = el.closest('.summary-footer, .main-header, .modal-wrapper, .room-options-menu, .fab-container');
    ...
    function buildOwnerSummary(payload) {
        ...
        payload.rooms.forEach((room, rIdx) => {
            if (room.is_suspended) {
                summary += `ห้อง ${room.room_name || rIdx + 1}: -- ระงับการใช้งาน --\n\n`;
                return;
            }
            let roomTotal = 0;
            summary += `### ห้อง: ${room.room_name || `ห้อง ${rIdx + 1}`} ###\n`;
            ...
            room.decorations.forEach((deco, dIdx) => {
                if (deco.is_suspended) {
                    summary += ` - ${deco.type || 'ตกแต่ง'} #${dIdx + 1}: -- ระงับ --\n`; return;
                }
    ...
    function toggleFabMenu(forceState) {
        if (forceState === false || fabContainer.classList.contains('active')) {
    ...
    function init() {
        ...
        document.body.addEventListener("click", e => {
            ...
            if (btn.closest('.fab-action-item')) toggleFabMenu(false);
            if (btn.closest('.room-options-menu')) btn.closest('.room-options-menu').classList.remove('show');
            ...
            const actions = {
                ...
                'toggle-room-menu': () => {
                    ...
                    document.querySelectorAll('.room-options-menu.show, .menu-dropdown.show').forEach(m => m.classList.remove('show'));
                    toggleFabMenu(false);
                    // If it wasn't open, open it now
                    if (!wasOpen) thisMenu?.classList.add('show');
                },
                'toggle-suspend-room': () => {
                    const roomEl = btn.closest(SELECTORS.room);
                    if (!roomEl) return;
                    suspendRoom(roomEl, !(roomEl.dataset.suspended === 'true'));
                },
        ...
        setupMenuAction(SELECTORS.copyTextBtn, async () => {
        ...
        setupMenuAction(SELECTORS.submitBtn, () => {
             document.querySelector(SELECTORS.orderForm).action = WEBHOOK_URL;
             document.querySelector(SELECTORS.payloadInput).value = JSON.stringify(buildPayload());
             showToast("ส่งข้อมูลแล้ว...", "success");
             // orderForm.submit();
        });

        setupMenuAction(SELECTORS.clearAllBtn, async () => {
        ...
        setupMenuAction(SELECTORS.importBtn, async () => {
            const payload = await showImportModal();
            if (payload) loadPayload(payload);
        });

        // --- NEW Pop-up Toggling Logic ---
        document.querySelector(SELECTORS.fabMainBtn).addEventListener('click', (e) => {
            ...
            // Toggle FAB
            toggleFabMenu(wasActive ? false : true);
        });
        ...
        window.addEventListener('click', (e) => {
            setActiveRoom(e.target.closest(SELECTORS.room));

            // Close popups when clicking outside of their containers
