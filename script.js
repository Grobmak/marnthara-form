...
orderForm.addEventListener("click", e => {
    ...
    const action = (btn.dataset.act || '').trim();
    const roomEl = btn.closest(SELECTORS.room);

    // Prevent default only for anchors that point to '#'
    ...
    // Toggle room menu (handled separately to avoid accidental close)
    if (action === 'toggle-room-menu') {
        const container = btn.closest('.room-options-container');
        const menu = container?.querySelector('.room-options-menu');
    ...
    const actions = {
        ...
        'clear-deco': () => performActionWithConfirmation(btn, { confirm: true, title: 'ล้างข้อมูล', body: 'ยืนยันการล้างข้อมูลในรายการนี้?', selector: SELECTORS.decoItem, action: (item) => { item.querySelectorAll('input, select').forEach(el => el.value = ''); const dt = item.querySelector('.deco-type-display'); if (dt) dt.textContent = ''; }, toast: 'ล้างข้อมูลตกแต่งแล้ว' }),
        'clear-wallpaper': () => performActionWithConfirmation(btn, { confirm: true, title: 'ล้างข้อมูล', body: 'ยืนยันการล้างข้อมูลในรายการนี้?', selector: SELECTORS.wallpaperItem, action: (item) => { item.querySelectorAll('input').forEach(el => { if (el.name === 'wallpaper_install_cost') el.value = '300'; else el.value = ''; }); item.querySelector(SELECTORS.wallsContainer).innerHTML = ''; addWall(item.querySelector('[data-act="add-wall"]')); }, toast: 'ล้างข้อมูลวอลเปเปอร์แล้ว' }),
    ...
    if (actions[action]) {
        actions[action]();
        // if the action originated from inside a room menu, close that menu after running action
        const roomMenu = btn.closest('.room-options-menu');
        if (roomMenu) roomMenu.classList.remove('show');
        // update UI and persistent state
        renumber();
        recalcAll();
        saveData();
    }
