(function() {
    ...
    function recalcAll() {
        ...
        let pricedItemCount = 0; // For counting items with price > 0
        const decoCounts = {};
        ...
        document.querySelectorAll(SELECTORS.room).forEach(room => {
            ...
            room.querySelectorAll(SELECTORS.set).forEach(set => {
                ...
                if(totalPrice > 0) pricedItemCount++;
            ...
            grand += roomSum;
            const activeItemsInRoom = Array.from(room.querySelectorAll(`${SELECTORS.set}, ${SELECTORS.decoItem}, ${SELECTORS.wallpaperItem}`)).filter(item => item.dataset.suspended !== 'true');
            room.querySelector('[data-room-brief]').innerHTML = `<span>${activeItemsInRoom.length} รายการ • ${fmt(roomSum, 0, true)} บาท</span>`;
        });
    ...
    function init() {
        ...
        orderForm.addEventListener("click", e => {
            ...
            const actions = {
                ...
                'del-wall': () => performActionWithConfirmation(btn, { selector: '.wall-input-row', action: (item) => item.remove() }),
                ...
                'clear-deco': () => performActionWithConfirmation(btn, { confirm: true, title: 'ล้างข้อมูล', body: 'ยืนยันการล้างข้อมูลในรายการนี้?', selector: SELECTORS.decoItem, action: (item) => item.querySelectorAll('input').forEach(el => el.value = ''), toast: 'ล้างข้อมูลตกแต่งแล้ว' }),
