    ...
    function recalcAll() {
        ...
        document.querySelectorAll(SELECTORS.room).forEach((room) => {
            ...
            room.querySelectorAll(SELECTORS.wallpaperItem).forEach(wallpaper => {
                ...
                let totalStrips = [...wallpaper.querySelectorAll('[name="wall_width_m"]')].reduce((sum, input) => {
                    const wallWidth = clamp01(input.value);
                    if (wallWidth <= 0 || h <= 0) return sum;
                    return sum + Math.ceil(wallWidth / WALLPAPER_ROLL_WIDTH_M);
                }, 0);
                
                const totalLengthNeeded = totalStrips * h;
