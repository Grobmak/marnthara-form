// Utility functions (can be copied from your main script.js)
...
const generateHTML = (data) => {
    ...
    return `
        ...
        <head>
            <meta charset="UTF-8">
            <style>
                /* Copy all CSS from the "--- PDF QUOTATION STYLES ---" section of your style.css file here */
                :root { --primary: #543D49; --on-primary: #FFFFFF; --outline-variant: #BCC2CB; }
                body { font-family: 'Noto Sans Thai', sans-serif; /* You might need to embed a font for full compatibility */ color: #333; font-size: 10pt; background-color: #fff; }
                #quotation-template { padding: 10mm 5mm; }
                /* ... (paste the rest of the PDF styles here) ... */
        ...
        <body>
            <div id="quotation-template">
                <header class="pdf-header">${logoHtml}...</header>
                <section class="pdf-customer-details">...</section>
                <table class="pdf-items-table"><tbody>${tableRows}</tbody></table>
                <section class="pdf-summary-section">
                    <div class="pdf-amount-in-words">( ${bahttext(grandTotal)} )</div>
                    <div class="pdf-totals-block"><table>${vatDisplay}<tr class="pdf-grand-total">...</tr></table></div>
                </section>
                <footer class="pdf-footer-section">...</footer>
            </div>
        </body>
        </html>
    `;
