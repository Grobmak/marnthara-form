...
function init() {
    ...
    // Note: The original file had a selector for '#addRoomFooterBtn' which doesn't exist in the HTML.
    // The button to add a room is inside the form, handled by handleFormClick.
    // So, I'm removing the redundant listener.
    ...
    document.querySelector(SELECTORS.exportPdfBtn).addEventListener('click', async (e) => {
        ...
        // ***** FIX: Refactored logic to use correct functions *****
        if (options.exportMethod === 'html') {
            exportAsHtmlFile(quotation);
        } else {
            // 'direct' and 'print' methods are handled by the renderPdf function
