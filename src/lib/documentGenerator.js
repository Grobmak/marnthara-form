...
// --- Grand Total Calculation Engine (Centralized) ---
function calculateGrandTotal(payload) {
    return payload.rooms.reduce((roomSum, room) => {
        if (room.is_suspended) return roomSum;

        const setsTotal = room.sets.reduce((sum, set) => sum + CALC.calculateSetPrice(set).total, 0);
        const decosTotal = room.decorations.reduce((sum, deco) => sum + CALC.calculateDecoPrice(deco).total, 0);
        const wpsTotal = room.wallpapers.reduce((sum, wp) => sum + CALC.calculateWallpaperPrice(wp).total, 0);
...
export function generateSummaryText(payload, type) {
    ...
    if (type === 'customer') {
        ...
        payload.rooms.forEach(room => {
            ...
            const activeItems = [...room.sets, ...room.decorations, ...room.wallpapers].filter(i => {
                 if (i.is_suspended) return false;
                 if (i.widths) return i.widths.reduce((a, b) => a + b, 0) > 0; // Wallpaper check
                 return i.width_m > 0; // Set and Deco check
            });
...
export function generateOverviewHtml(payload) {
    ...
    payload.rooms.forEach(room => {
        ...
        room.sets.forEach(set => {
            if (CALC.calculateSetPrice(set).total > 0) {
                const key = `ผ้าม่าน`;
        ...
        room.decorations.forEach(deco => {
            if (deco.type && CALC.calculateDecoPrice(deco).total > 0) {
                const key = deco.type;
                roomSummary[key] = (roomSummary[key] || 0) + 1;
                grandTotalSummary[key] = (grandTotalSummary[key] || 0) + 1;
            }
        ...
        room.wallpapers.forEach(wp => {
            if (CALC.calculateWallpaperPrice(wp).total > 0) {
                const key = 'วอลเปเปอร์';
                roomSummary[key] = (roomSummary[key] || 0) + 1;
                grandTotalSummary[key] = (grandTotalSummary[key] || 0) + 1;
            }
        });

        const itemEntries = Object.entries(roomSummary);
        if (itemEntries.length > 0) {
            tableBodyHtml += `<tr class="room-header-row"><td colspan="3">${room.room_name || 'ไม่ระบุชื่อห้อง'}</td></tr>`;
            itemEntries.forEach(([type, count]) => {
                tableBodyHtml += `<tr><td></td><td class="item-type-cell">${type}</td><td class="pdf-text-center">${count}</td></tr>`;
    ...
    const summaryText = Object.entries(grandTotalSummary)
        .map(([type, count]) => `${type}: ${count}`)
        .join(' , ');

    let finalHtml = `<div id="detailed-material-summary">
        <h4><i class="ph ph-stack"></i> ยอดรวมทุกประเภท</h4>
        <ul><li class="summary-note">${summaryText}</li></ul>
    </div>`;
...
export function generateQuotationHtml(payload, options) {
    ...
    payload.rooms.forEach(room => {
        ...
        room.sets.forEach(set => {
            const totalSetPrice = CALC.calculateSetPrice(set).total;
            if (totalSetPrice > 0) {
        ...
        room.decorations.forEach(deco => {
            const decoPrice = CALC.calculateDecoPrice(deco).total;
            if (decoPrice > 0) {
        ...
        room.wallpapers.forEach(wp => {
            const wpPrice = CALC.calculateWallpaperPrice(wp).total;
            if (wpPrice > 0) {
                const totalWidth = wp.widths.reduce((a, b) => a + b, 0);
                const rolls = CALC.wallpaperRolls(totalWidth, wp.height_m);
