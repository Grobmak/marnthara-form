...
export const CALC = {
     ...
     * @returns {number} The calculated total price for the set.
     */
    calculateSetPrice: (set) => {
        if (!set || set.is_suspended || toNum(set.width_m) <= 0) return 0;
        const sPlus = stylePlus(set.style);
        const hPlus = heightPlus(toNum(set.height_m));
        ...
        return Math.round(opaquePrice + sheerPrice);
    },

    /**
     * Calculates the total price for a single decoration item.
     * @param {object} deco - A decoration object.
     * @returns {number} The calculated total price for the decoration.
     */
    calculateDecoPrice: (deco) => {
        if (!deco || deco.is_suspended || toNum(deco.width_m) <= 0) return 0;
        const totalSqm = toNum(deco.width_m) * toNum(deco.height_m);
        const price = totalSqm * SQM_TO_SQYD * toNum(deco.price_sqyd);
     ...
     * @returns {number} The calculated total price for the wallpaper.
     */
    calculateWallpaperPrice: (wp) => {
        if (!wp || wp.is_suspended) return 0;
        const totalWidth = wp.widths?.reduce((sum, w) => sum + toNum(w), 0) || 0;
        if (totalWidth <= 0) return 0;

        const rolls = CALC.wallpaperRolls(totalWidth, toNum(wp.height_m));
        const materialPrice = rolls * toNum(wp.price_per_roll);
        const installPrice = rolls * (toNum(wp.install_cost_per_roll) || 0);
        return Math.round(materialPrice + installPrice);
    }
};